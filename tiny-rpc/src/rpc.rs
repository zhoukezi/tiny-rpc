...
pub trait RpcServerStub<R: Rpc, I: RpcFrame<R::Request>, O: RpcFrame<R::Response>> {
    fn make_response(
        self: Arc<Self>,
        req: I,
        rsp_handler: ResponseHandler<R::Response, O>,
...
#[derive(Debug)]
pub struct ResponseHandler<T, F: RpcFrame<T>>(mpsc::Sender<F>, PhantomData<T>);

impl<T, F: RpcFrame<T>> ResponseHandler<T, F> {
    pub async fn response_with(mut self, rsp: F) {
        self.0.send(rsp).await.expect("driver closed unexpectedly")
    }
}

pub trait RpcFrame<T>: Send + 'static {
    fn new(id: RequestId, data: T) -> Self;
    fn get_id(&self) -> RequestId;
    fn get_data(self) -> T;
}

impl<T: Send + 'static> RpcFrame<T> for (RequestId, T) {
    fn new(id: RequestId, data: T) -> Self {
        (id, data)
    }
    ...
    loop {
        match fut.await {
            Either::Left((Some(req), r)) => {
                let stub = stub.clone();
                tokio::spawn(stub.make_response(req, ResponseHandler(tx.clone(), PhantomData)));
